...
<Steps>
  <Step title="Open">
    ...
    `detected_language` reflects the BCP-47 code the session will actually use — useful when
    the requested language is not configured on the kiosk and the server falls back to its
    default. `disclosure_text` is the EU AI Act disclosure string for this kiosk; display it
    to the end user before or at the start of the conversation. Both fields are absent when
    not applicable (single-language kiosk, no disclosure configured).

    See [Language support](/concepts/language-support) for how to send `preferred_language`
    and how to render the disclosure banner.
  </Step>
  <Step title="Interact">
