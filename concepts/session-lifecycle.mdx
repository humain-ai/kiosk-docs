...
<Steps>
  ...
  <Step title="Interact">
    While the session is open, the device sends messages (text mode) or streams audio (voice mode).
    Each turn runs through the full pipeline:
...
- A background worker runs every **30 minutes**.
- Sessions with no activity for 30 minutes are marked abandoned and closed.
...
| Auto-abandonment | Per session | 30 minutes of inactivity |
